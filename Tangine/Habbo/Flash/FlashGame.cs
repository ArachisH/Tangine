...
public class FlashGame : HGame
{
    ...
    protected FlashGame(ShockwaveFlash flash, string path)
        : base(GamePatches.DisableHostChecks | GamePatches.DisableEncryption | GamePatches.InjectKeyShouter | GamePatches.InjectEndPointShouter | GamePatches.InjectEndPoint)
    {
        _flash = flash;
    ...
    {
        GamePatches.InjectEndPoint => InjectEndPoint(),
    ...
    private bool InjectEndPoint()
    {
        ...
        for (int i = 0; i < initCode.Count; i++)
        {
            ...
            {
                initCode[i - 2] = new PushStringIns(_socketConnectionInstance.ABC, InjectableEndPoint.Address.ToString());
                initCode[i - 1] = new PushIntIns(_socketConnectionInstance.ABC, InjectableEndPoint.Port);
            }
        }
